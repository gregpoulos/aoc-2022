...
for i, bp in enumerate(blueprints[:3]):
  if bp_id and i+1 != bp_id: continue
  strategies = []
  earliest_build_time = 15
  ...
  print(f'Trying {len(strategies)} strategies out for Blueprint #{i+1}')

  for z, s in enumerate(strategies):
    if z % 10000 == 0: print(f'... tried {z} strategies')
