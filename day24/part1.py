...
class Library():
  ...
  def at_minute(self, t):
    return (self.states[t % len(self)], t % len(self))
...
all_states = Library(parse(fn), quiet)
...
for t in range(len(all_states)):
  ...
  exit = valley.exit()
  if valley.is_open(exit):
    path_lengths[(exit, _id)] = 0
    backlog.append((exit, _id))
