...
def circumference(center, distance):
  here = (center[0]-distance, center[1])
  for i in range(distance):
    yield here
    here = (here[0]+1, here[1]-1)
  for i in range(distance):
    yield here
    here = (here[0]+1, here[1]+1)
  for i in range(distance):
...
DISTRESS_MAX_T = 20
def in_range(pt):
  return pt[0] >= 0 and pt[0] <= DISTRESS_MAX and pt[1] >= 0 and pt[1] <= DISTRESS_MAX

circumferences = []
g = Grid()
counter = 0
for b, s in zip(beacons, sensors):
  print(f'Processing sensor #{counter}')
  g.nearest_beacon_by_sensor[s] = b
  candidates = filter(in_range, circumference(s, Grid.distance(b, s)+1))
  circumferences.append(set(candidates))
  counter += 1

counter = 0
candidates = set()
for c1 in circumferences:
  for c2 in circumferences:
    if c1 != c2:
      candidates.update(c1.intersection(c2))
  print(f'Finding intersections for sensor #{counter}')
  counter += 1

distress_beacon = ()
for c in candidates:
  if not g.is_excluded(c):
    distress_beacon = c
    print(c)
    break
print(distress_beacon[0] * DISTRESS_MAX + distress_beacon[1])
